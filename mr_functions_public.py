...
def fit_with_covs(df, covs,
                  resp_name, se_name, study_id_name,
                  data_id_name=None,
                  ## default gives just random intercepts and not random slopes
                  ## current version of function will always include a random intercept
                  z_covs=['intercept'],
                  ## been using zero here because we dropping outliers earlier (in MRBRT)
                  # spline_cov managed through a different function (step 2)
                  trim_prop=0.0, spline_cov=None, spline_degree=None,
                  spline_knots_type=None, spline_knots=None,
                  spline_monotonicity=None, ensemble=False,
                  full_spline_basis=False,
                  ## Do specify from here down
                  # crosswalks are input into this step here
                  gprior_dict=None,
                  ## might not be using uprior_dict 
                  ## if there is a covariate that must have certain relationship with log_icer
                  ## could include here to constrain the relationship
                  uprior_dict=None,
                  ## don't specify these in this step 
    ...
    if spline_cov is not None:
        ...
        if ensemble:
            sys.exit("Wrapper for ensemble splines not yet working")
        else:
...
def cv_gaussian_prior(df, resp_name, se_name, covs,
    ...
    stdized_df[covs_to_stdize] = (stdized_df[covs_to_stdize] - stdized_df[covs_to_stdize].mean(axis=0)) / stdized_df[covs_to_stdize].std(axis=0)
    
    mask_list = [stdized_df[fold_id_name] == fid
            for fid in stdized_df[fold_id_name].unique()]
    
    train_test_mrd_list = [tuple(mrtool.MRData(obs=stdized_df.loc[m == truth_val, resp_name].values,
    ...
    while upper_prior_sd - lower_prior_sd > sd_tol:
        ...
        if combine_gpriors:
            gprior_cov_model_lists = [
                    [mrtool.LinearCovModel(key, use_re=False,
                                           prior_beta_gaussian=np.array([val[0] / (1 + (val[1]**2 / sd**2)),
        ...
        cov_model_lists = [[mrtool.LinearCovModel('intercept', use_re=True)] + \
                           [mrtool.LinearCovModel(v, use_re=False, prior_beta_gaussian=np.array([0, sd]))
                                for v in covs if v not in ['intercept'] + list(constrained_covs.keys()) + list(beta_gpriors.keys())] +\
...
def r2(mr, fit_df, resp_name):
    ...
    r2s = (fit_df[[resp_name, 'fitted_fe_and_re', 'fitted_fe_only']].corr()**2).loc[resp_name, ['fitted_fe_and_re', 'fitted_fe_only']]
    r2s.name = 'R_squared'
...
def create_predictions(mr, signal_mr, preds_df,
    ...
    if 'new_spline_cov' not in preds_df.columns:
        signal_preds_df = create_signal(signal_mr, spline_cov, preds_df[spline_cov].to_numpy(),
    ...
    np.random.seed(seed)
    # If provided, use the same samples of beta as those used for UIs of betas. Otherwise, sample them.
    if beta_samples is None:
        beta_samples = mrtool.core.other_sampling.sample_simple_lme_beta(n_samples,
                                                                         mr)
